...
def build_readme(dry_run: bool = False, verbose: bool = False) -> str:
    ...
    print("\n◈ Generating SVG assets (only flow-line) ...")
    ...
    readme_content = readme_content.replace("49f2eb37858ef4a0", build_hash[:16])
    readme_content = readme_content.replace("2026-02-21T21:00:00Z", timestamp)
    readme_content = readme_content.replace("a3f8c2d1e9b7", build_hash[:12])
    readme_content = readme_content.replace("2026-02-21T22:15:00Z", timestamp)
    readme_content = readme_content.replace("7b2e9f4a1c8d", build_hash[:12])
    readme_content = readme_content.replace("c9d3e8f2a5b1", build_hash[:12])
    if not dry_run:
