#!/usr/bin/env python3
import os

def generate_glitch_snake():
    glyph = "𖢧ꛅ𖤢 ꚽꚳꛈ𖢧ꛕꛅ"
    svg = f"""<svg width="1000" height="200" viewBox="0 0 1000 200" xmlns="http://www.w3.org/2000/svg">
  <rect width="1000" height="200" fill="#0d1117"/>
  <defs>
    <filter id="f1">
      <feTurbulence type="fractalNoise" baseFrequency="0.01 0.4" numOctaves="1" result="warp">
        <animate attributeName="baseFrequency" values="0.01 0.4;0.05 0.2;0.01 0.4" dur="0.1s" repeatCount="indefinite" />
      </feTurbulence>
      <feDisplacementMap xChannelSelector="R" yChannelSelector="G" scale="30" in="SourceGraphic" in2="warp" />
    </filter>
  </defs>

  <g opacity="0.4">
    <text x="0" y="180" font-family="monospace" font-size="14" fill="#4ade80">
      <tspan>🜂 ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─ ─ ◈ ─</tspan>
      <animateTransform attributeName="transform" type="translate" from="0 0" to="-200 0" dur="3s" repeatCount="indefinite" />
    </text>
  </g>

  <g font-family="monospace" font-size="64" font-weight="bold" text-anchor="middle" dominant-baseline="middle">
    <text x="502" y="100" fill="#ff0000" opacity="0.6" filter="url(#f1)">{glyph}</text>
    <text x="498" y="100" fill="#00f3ff" opacity="0.6" filter="url(#f1)">{glyph}</text>
    <text x="500" y="100" fill="#ffffff">{glyph}</text>
  </g>
</svg>"""
    os.makedirs('assets', exist_ok=True)
    with open('assets/glitch_snake.svg', 'w') as f:
        f.write(svg)
    print("◈ Spectre-grade Glitch Snake generated.")

if __name__ == "__main__":
    generate_glitch_snake()
