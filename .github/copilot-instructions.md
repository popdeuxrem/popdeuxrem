# GitHub Copilot instructions — popdeuxrem (Quantum Surface)
...
## Quick summary
- Canonical output: `README.md` is auto-generated by `scripts/update_readme.py` and should be treated as a build artifact, not hand-edited. ✅
...
- `scripts/update_readme.py` — orchestrates generation steps and writes `README.md`.
  - Example: runs `gen_*()` functions from `scripts/design_engine.py` then writes README with a build hash and timestamp.
- `scripts/design_engine.py` — creates `assets/section_*.svg` via `write_svg()` helpers. Add or modify `gen_*()` functions to change visual sections.
- `scripts/discovery.py` — collects GitHub metadata (uses optional `GITHUB_TOKEN`) and writes `dist/telemetry_data.json`.
- `scripts/generate_vessel_reports.py` — writes per-repo markdown in `docs/deployments/` and an `INDEX.md`.
- `scripts/vuln_scan.sh` — uses `pip-audit` to populate `dist/security-report.json`; the workflow includes `pip install pip-audit` and runs this as part of CI.

## Developer workflows (exact commands)
- Quick local build: `bash scripts/build.sh` (sets exec bits, runs cleanup if present, and regenerates `README.md`). 🔧
...
- Generate vessel reports: `python3 scripts/generate_vessel_reports.py` → outputs `docs/deployments/*.md` and `docs/deployments/INDEX.md`.
- Security scan (local): `bash scripts/vuln_scan.sh` → outputs `dist/audit_raw.json` and `dist/security-report.json`.
...
- `skills.json` can be either a dict (category → list) or a list; `scripts/discovery.py::render_skill_bars()` handles both but expect inconsistent shapes in changes.
...
1. If the change affects `README.md` content, update the generator (not `README.md` directly). Reference: `scripts/update_readme.py` and `scripts/design_engine.py`. ✅
2. Run `bash scripts/build.sh` locally and validate outputs in `assets/` and `dist/`. ✅
3. If adding or changing telemetry or audit output shapes, update any consumer (`README` generation, `generate_vessel_reports.py`, CI push logic). ✅
4. Add a short note in `docs/ARCHITECTURE.md` if you introduce new runtime components or scheduled tasks. Keep notes factual and discoverable. ✅
